...
def choose_source_file():
    file = filedialog.askopenfilename(
        ...
        filetypes=(("All Files", "*.*"), ("Images", "*.png;*.jpg;*.jpeg;*.gif"), ("Videos", "*.mp4"))
    )
...
def convert_files():
    ...
    for i, file in enumerate(files, 1):
        ...
        if file.endswith(('png', 'jpg', 'jpeg')):
            subprocess.run(['ffmpeg', '-i', file, '-vcodec', 'libwebp', '-lossless', '0', '-compression_level', '6', '-q:v', '80', output_file])
        elif file.endswith('gif'):
            subprocess.run(['ffmpeg', '-i', file, '-vcodec', 'libwebp', '-vf', 'scale=640:-1', '-lossless', '0', '-compression_level', '6', '-q:v', '50', '-loop', '0', output_file])
        elif file.endswith('mp4'):
            subprocess.run(['ffmpeg', '-i', file, '-vcodec', 'libwebp', '-vf', 'scale=640:-1,fps=15', '-lossless', '0', '-compression_level', '6', '-q:v', '50', '-loop', '0', '-an', '-vsync', '0', output_file])
...
root.geometry("750x500")  # تحديد حجم الواجهة
root.resizable(width=False, height=False)  # تعيين قابلية التكبير والتصغير إلى القيمة False

# إضافة صورة خلفية
background_image = Image.open("C:/Users/123/Desktop/GPT/AI.jpg")  # فتح صورة الخلفية
background_image = background_image.resize((750, 500), Image.BICUBIC)  # تغيير حجم الصورة
...
root.geometry(f"750x500+{x_position}+{y_position}")
...
tk.Button(fram, text="اختيار المجلد", font=("Helvetica", 14), command=choose_destination_folder, width=10, height=1, bg="#e74c3c", fg="white").grid(row=4, column=1, padx=10, pady=10)

# زر التحويل
tk.Button(fram, text="بدء التحويل", font=("Helvetica", 14), width=25, height=1, bg="#3498db", fg="white", command=convert_files).grid(row=5, column=0, padx=10, pady=20)
